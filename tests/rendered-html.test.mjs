...
test("keeps saved looks and styling recommendations connected to the product", async () => {
  ...
  assert.match(page, /¿Cómo visto mi semana\?/);
  assert.match(page, /Quiero usar más mi closet/);
  ...
  assert.match(page, /PARA RESPUESTAS MÁS EXACTAS/);
  assert.match(page, /RESPUESTA PERSONALIZADA/);
